...
class ref_plant_cat extends MX_Controller  {
	
	var $table = "ref_plant_cat";
	var $table_alias = "Plant category";	
	var $uri_page = 6;
	var $per_page = 25;
	...
	{	
		...
		$jum_record = $this->ref_plant_cat->getTotal($this->table,$sch1_parm,$sch2_parm);
		...
		#record
		$query = $this->ref_plant_cat->getList($this->table,$per_page,$lmt,$sch1_parm,$sch2_parm);
		$list = array();
	...
	function edit_content($id)
	{
		...
		if(is_numeric($id)){
			...
			#record
			$q = $this->ref_plant_cat->getDetail($this->table,$id);
			...
			if($q->num_rows() > 0){
				foreach($q->result() as $r){
					...
					$desc = $this->session->flashdata("desc_") ? $this->session->flashdata("desc_") : "";
	...
	function submit()
	{
		...
		if ($this->form_validation->run($this) == FALSE)
		{
			...
			if($id > 0)
			{
				$this->ref_plant_cat->setUpdate($this->table,$id,$title,$desc_,$ref2,$user_id);
				$this->session->set_flashdata("success","Data saved successful");
				redirect($this->table."/edit/".$id);
			}else{
				$id_term = $this->ref_plant_cat->setInsert($this->table,$id,$title,$desc_,$ref2,$user_id);
	...
	{
		$del_status = $this->ref_plant_cat->setDelete($this->table,$id);
		$response['id'] = $id;
	...
	function getRefDropdown($id,$name,$type=NULL)
	{
		...
		$q = $this->ref_plant_cat->getDropdown($this->table);
		$list = array();

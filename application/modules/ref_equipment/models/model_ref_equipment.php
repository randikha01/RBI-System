<?php
class Model_ref_equipment extends CI_Model {
	...
	function getTotal($table,$sch1_parm,$sch2_parm,$sch3_parm)
	{
		...
		if($sch3_parm != 'null' && !empty($sch3_parm) )
		{
			$this->db->like($table.'.ref_code',$sch3_parm);
		}
	...
	function getList($table,$per_page,$lmt,$sch1_parm,$sch2_parm,$sch3_parm)
	{
		if($sch1_parm != 'null' && !empty($sch1_parm) )
		{
			$this->db->like($table.'.title',$sch1_parm);
		...
		$this->db->order_by($table.".ref_code","asc");

...
class ref_equipment extends MX_Controller  {
	...
	function submit()
	{
		...
		if ($this->form_validation->run($this) == FALSE)
		{
			...
			if($id > 0)
			{
				$this->ref_equipment->setUpdate($this->table,$id,$ref_code,$ref_title,$ref2,$user_id);
				$this->session->set_flashdata("success","Data saved successful");
				redirect($this->table."/edit/".$id);
			}else{
				$id_term = $this->ref_equipment->setInsert($this->table,$id,$ref_code,$ref_title,$ref2,$user_id);

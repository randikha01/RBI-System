...
class ref_formula extends MX_Controller  {
	
	var $table = "ref_formula";
	var $table_alias = "Formula";	
	var $uri_page = 6;
	var $per_page = 25;
	...
	{	
		...
		$jum_record = $this->ref_formula->getTotal($this->table,$sch1_parm,$sch2_parm);
		...
		#record
		$query = $this->ref_formula->getList($this->table,$per_page,$lmt,$sch1_parm,$sch2_parm);
		$list = array();
	...
	function edit_content($id)
	{
		...
		if(is_numeric($id)){
			...
			#record
			$q = $this->ref_formula->getDetail($this->table,$id);
	...
	function submit()
	{
		...
		if ($this->form_validation->run($this) == FALSE)
		{
			...
			if($id > 0)
			{
				$this->ref_formula->setUpdate($this->table,$id,$desc_,$title,$ref2,$user_id);
				$this->session->set_flashdata("success","Data saved successful");
				redirect($this->table."/edit/".$id);
			}else{
				$id_term = $this->ref_formula->setInsert($this->table,$id,$desc_,$title,$ref2,$user_id);
	...
	{
		$del_status = $this->ref_formula->setDelete($this->table,$id);
		$response['id'] = $id;

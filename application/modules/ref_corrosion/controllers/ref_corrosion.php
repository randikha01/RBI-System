...
class ref_corrosion extends MX_Controller  {
	
	var $table = "ref_corrosion";
	var $table_alias = "Corrosion Control";	
	var $uri_page = 6;
	var $per_page = 25;
	...
	{	
		...
		$jum_record = $this->ref_corrosion->getTotal($this->table,$sch1_parm,$sch2_parm);
		...
		#record
		$query = $this->ref_corrosion->getList($this->table,$per_page,$lmt,$sch1_parm,$sch2_parm);
		$list = array();
	...
	function edit_content($id)
	{
		...
		if(is_numeric($id)){
			...
			#record
			$q = $this->ref_corrosion->getDetail($this->table,$id);
	...
	function submit()
	{
		...
		if ($this->form_validation->run($this) == FALSE)
		{
			...
			if($id > 0)
			{
				$this->ref_corrosion->setUpdate($this->table,$id,$title,$desc_,$ref2,$user_id);
				$this->session->set_flashdata("success","Data saved successful");
				redirect($this->table."/edit/".$id);
			}else{
				$id_term = $this->ref_corrosion->setInsert($this->table,$id,$title,$desc_,$ref2,$user_id);
	...
	{
		$del_status = $this->ref_corrosion->setDelete($this->table,$id);
		$response['id'] = $id;
	...
	function getRefDropdown($id,$name,$type=NULL)
	{
		...
		$q = $this->ref_corrosion->getDropdown($this->table);
		$list = array();

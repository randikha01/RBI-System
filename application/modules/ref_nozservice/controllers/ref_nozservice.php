...
class ref_nozservice extends MX_Controller  {
	
	var $table = "ref_nozservice";
	var $table_alias = "Nozzle Service";	
	var $uri_page = 6;
	var $per_page = 25;
	...
	{	
		...
		$jum_record = $this->ref_nozservice->getTotal($this->table,$sch1_parm,$sch2_parm);
		...
		#record
		$query = $this->ref_nozservice->getList($this->table,$per_page,$lmt,$sch1_parm,$sch2_parm);
		$list = array();
	...
	function edit_content($id)
	{
		...
		if(is_numeric($id)){
			...
			#record
			$q = $this->ref_nozservice->getDetail($this->table,$id);
	...
	function submit()
	{
		...
		if ($this->form_validation->run($this) == FALSE)
		{
			...
			if($id > 0)
			{
				$this->ref_nozservice->setUpdate($this->table,$id,$desc_,$title,$ref2,$user_id);
				$this->session->set_flashdata("success","Data saved successful");
				redirect($this->table."/edit/".$id);
			}else{
				$id_term = $this->ref_nozservice->setInsert($this->table,$id,$desc_,$title,$ref2,$user_id);
	...
	{
		$del_status = $this->ref_nozservice->setDelete($this->table,$id);
		$response['id'] = $id;

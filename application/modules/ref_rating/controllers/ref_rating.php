...
class ref_rating extends MX_Controller  {
	
	var $table = "ref_rating";
	var $table_alias = "Rating";	
	var $uri_page = 6;
	var $per_page = 25;
	...
	{	
		...
		$jum_record = $this->ref_rating->getTotal($this->table,$sch1_parm,$sch2_parm);
		...
		#record
		$query = $this->ref_rating->getList($this->table,$per_page,$lmt,$sch1_parm,$sch2_parm);
		$list = array();
	...
	function edit_content($id)
	{
		...
		if(is_numeric($id)){
			...
			#record
			$q = $this->ref_rating->getDetail($this->table,$id);
	...
	function submit()
	{
		...
		if ($this->form_validation->run($this) == FALSE)
		{
			...
			if($id > 0)
			{
				$this->ref_rating->setUpdate($this->table,$id,$desc_,$title,$ref2,$user_id);
				$this->session->set_flashdata("success","Data saved successful");
				redirect($this->table."/edit/".$id);
			}else{
				$id_term = $this->ref_rating->setInsert($this->table,$id,$desc_,$title,$ref2,$user_id);
	...
	{
		$del_status = $this->ref_rating->setDelete($this->table,$id);
		$response['id'] = $id;

...
class ref_corrosion_loop extends MX_Controller  {
	
	var $table = "ref_corrosion_loop";
	var $table_alias = "Corrosion";	
	var $uri_page = 6;
	var $per_page = 25;
	...
	{	
		...
		$jum_record = $this->ref_corrosion_loop->getTotal($this->table,$sch1_parm,$sch2_parm);
		...
		#record
		$query = $this->ref_corrosion_loop->getList($this->table,$per_page,$lmt,$sch1_parm,$sch2_parm);
		$list = array();
	...
	function edit_content($id)
	{
		...
		if(is_numeric($id)){
			...
			#record
			$q = $this->ref_corrosion_loop->getDetail($this->table,$id);
	...
	function submit()
	{
		...
		if ($this->form_validation->run($this) == FALSE)
		{
			...
			if($id > 0)
			{
				$this->ref_corrosion_loop->setUpdate($this->table,$id,$desc_,$title,$ref2,$user_id);
				$this->session->set_flashdata("success","Data saved successful");
				redirect($this->table."/edit/".$id);
			}else{
				$id_term = $this->ref_corrosion_loop->setInsert($this->table,$id,$desc_,$title,$ref2,$user_id);
	...
	{
		$del_status = $this->ref_corrosion_loop->setDelete($this->table,$id);
		$response['id'] = $id;

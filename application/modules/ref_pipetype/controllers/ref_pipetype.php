...
class ref_pipetype extends MX_Controller  {
	
	var $table = "ref_pipetype";
	var $table_alias = "Pipe Type";	
	var $uri_page = 7;
	var $per_page = 25;
	...
	{	
		...
		$jum_record = $this->ref_pipetype->getTotal($this->table,$sch1_parm,$sch2_parm,$sch3_parm);
		...
		#record
		$query = $this->ref_pipetype->getList($this->table,$per_page,$lmt,$sch1_parm,$sch2_parm,$sch3_parm);
		$list = array();
	...
	function edit_content($id)
	{
		...
		if(is_numeric($id)){
			...
			#record
			$q = $this->ref_pipetype->getDetail($this->table,$id);
			$list = $list_term_option = array();
			if($q->num_rows() > 0){
				foreach($q->result() as $r){
					
					$code = $this->session->flashdata("ref_code") ? $this->session->flashdata("ref_code") : $r->ref_code;
					$title = $this->session->flashdata("ref_title") ? $this->session->flashdata("ref_title") : $r->ref_title;
	...
	function submit()
	{
		...
		$ref_title = strip_tags($this->input->post("ref_title"));
		$ref2 = $this->input->post("ref2");
		$id = $this->input->post("id");
		$user_id = $this->session->userdata('adminID');

		$this->load->library('form_validation');
		$this->form_validation->set_rules('ref_title', 'title', 'required');
	
		if ($this->form_validation->run($this) == FALSE)
		{
			$this->session->set_flashdata("err",validation_errors());
			$this->session->set_flashdata("ref_code",$ref_code);
			$this->session->set_flashdata("ref_title",$ref_title);
			$this->session->set_flashdata("ref2",$ref2);
			redirect($this->table."/edit/".$id);
		}else{
			if($id > 0)
			{
				$this->ref_pipetype->setUpdate($this->table,$id,$ref_code,$ref_title,$ref2,$user_id);
				$this->session->set_flashdata("success","Data saved successful");
				redirect($this->table."/edit/".$id);
			}else{
				$id_term = $this->ref_pipetype->setInsert($this->table,$id,$ref_code,$ref_title,$ref2,$user_id);
	...
	{
		$del_status = $this->ref_pipetype->setDelete($this->table,$id);
		$response['id'] = $id;
	...
	function getRefDropdown($id,$name,$type=NULL)
	{
		...
		$q = $this->ref_pipetype->getDropdown($this->table);
		$list = array();
		foreach ($q->result() as $val) {
			$selected = $val->id == $id ? $selected = "selected='selected'" : "";	
			$list[]= array(
						'id' => $val->id,
						'title'=>ucfirst($val->ref_title),
						"selected"=>$selected
					 );
		}
		$data = array(
